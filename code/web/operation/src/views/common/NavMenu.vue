<template>
  <div class="navMenu">
    <template v-for="model in menuTree">
      <el-submenu v-if="model.children.length!=0" :key="model.menuId" :data="model" :index="model.menuName">
        <template slot="title">
      ...
      <el-menu-item v-if="model.children==0" :key="model.menuId" :data="model" :index="model.url"
                    :route="model.url">
        <i :class="model.icon" :style="{fontSize:iconSize}"></i>
...
<script scoped>
  export default {
    name: "NavMenu",
    props: ["menuTree", "iconSize", "colltitle"]
  };
</script>
