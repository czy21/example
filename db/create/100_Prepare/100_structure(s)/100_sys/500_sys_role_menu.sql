CREATE TABLE `sys_role_menu`  (
  ...
  CONSTRAINT `fk_RoleMenu_Menu` FOREIGN KEY (`MenuId`) REFERENCES `sys_menu` (`MenuId`) ON DELETE NO ACTION ON UPDATE RESTRICT,
  CONSTRAINT `fk_RoleMenu_Role` FOREIGN KEY (`RoleId`) REFERENCES `sys_role` (`RoleId`) ON DELETE NO ACTION ON UPDATE RESTRICT
) COMMENT='角色菜单表';